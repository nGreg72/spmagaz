...
{
	header('Location: '.forum_link($forum_url['index']));if (!defined('FORUM_ROOT'))
	exit;
}
...
else if (isset($_POST['form_sent']))
{
	...
	if (empty($errors))
	{
        ...
        $phoneNum = forum_trim($_POST['req_phoneNum']);
        $eField = forum_trim($_POST['eField']);
		...
		if (empty($errors))
		{
			...
			// phone
			$phone = (isset($_POST['phone']) && intval($_POST['phone']) === 1) ? 1 : $forum_config['o_default_phone'];
			...
			forum_setcookie($cookie_name, base64_encode($new_uid.'|'.$password_hash.'|'.$expire.'|'.sha1($salt.$password_hash.forum_hash($expire, $salt))), $expire);
			  ...
			  $cookie = base64_encode(serialize(array('uname'=>$username,'password'=>$password1)));
			  $a = setcookie('ckSavePass', $cookie,time()+2592000, '/', '');

header('Location: /com/login/?event=success');exit;
//
...
<div class="reg_field">
    <?php   if (!empty($forum_page['frm_info'])):?>
		<div class="ct-box info-box" style="background-color: azure;">
                    <?php echo implode("\n\t\t\t", $forum_page['frm_info'])."\n"
    ?>
		</div>
    <?php   endif;
...
<form class="frm-form frm-suggest-username" id="afocus" method="post" accept-charset="utf-8" action="<?php echo $forum_page['form_action'] ?>" autocomplete="off">
        <div class="hidden">
            ...
            <input type="hidden" name="phone" id="register_phone" value="<?php echo forum_htmlencode($forum_config['o_default_phone']) ?>" />
        </div>

    <?php ($hook = get_hook('rg_register_pre_group')) ? eval($hook) : null; ?>
    <div class="frm-group group<?php echo ++$forum_page['group_count'] ?>">

<!--Забивание электронной почты россии-->				
    <?php ($hook = get_hook('rg_register_pre_email')) ? eval($hook) : null; ?>
    <div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
            <div class="sf-box text required">
                <label for="fld<?php echo ++$forum_page['fld_count'] ?>">
                    <span><?php echo $lang_profile['E-mail'] ?></span>
                    <small><?php echo $lang_profile['E-mail help'] ?></small>
                </label><br/>
                <span class="fld-input">
                    <input type="email" data-suggest-role="email" id="email" 
                           name="req_email1" value="<?php echo(isset($_POST['req_email1']) ? forum_htmlencode($_POST['req_email1']) : '') ?>"
                           size="35" maxlength="80" required spellcheck="false" />
                </span>
            </div>
    </div>
    
        
<!-- Придумывание Логина пользователя при регистрации -->				
    ...
    <div class="sf-set set<?php echo ++$forum_page['item_count']; if ($forum_config['o_regs_verify'] == '0') echo ' prepend-top'; ?>">					
            <div class="sf-box text required">
                <label for="fld <?php echo ++$forum_page['fld_count']?>">
                    <span><?php echo $lang_profile['Username'] ?></span> 
                    <small><?php echo $lang_profile['Username help'] ?></small>
                </label><br/>
                <span class="fld-input">
                    <input type="text" data-suggest-role="username" id="name" 
                           name="req_username" value="<?php echo(isset($_POST['req_username']) ? forum_htmlencode($_POST['req_username']) : '') ?>" 
                           size="35" maxlength="25" required spellcheck="false" /></span>
            </div>
    </div>

<!--Забивание реального имени-->
    <?php ($hook = get_hook('rg_register_pre_realname')) ? eval($hook) : null; ?>
    <div class="sf-set set<?php echo ++$forum_page['item_count']; if ($forum_config['o_regs_verify'] == '0') echo ' prepend-top'; ?>">        
            <div class="sf-box text required">
                <label for="fld <?php echo ++$forum_page['fld_count']?>">
                    <span><?php echo $lang_profile['realName'] ?></span>
                    <small><?php echo $lang_profile['realName help'] ?></small>
                </label>    
                <span class="fld-input">    
                    <input type="text" data-suggest-role="realname" id = "realName" 
                           name="req_realname" value="<?php echo(isset($_POST['req_realname']) ? forum_htmlencode($_POST['req_realname']) : '') ?>"
                           size="35" maxlength="32" required spellcheck="false" />
                </span>
            </div>
    </div>

<!--Забивание реального номера телефона-->
    <?php ($hook = get_hook('rg_register_pre_phoneNum')) ? eval($hook) : null; ?>
    <div class="sf-set set<?php echo ++$forum_page['item_count']; if ($forum_config['o_regs_verify'] == '0') echo ' prepend-top'; ?>">
            <div class="sf-box text required">
                <label><span><?php echo $lang_profile['realPhone'] ?></span>
                        <small><?php echo $lang_profile['realPhone help'] ?></small>
                </label>
                <span class="fld-input">
                    <input type="text" id="checkPhone"
                           name="req_phoneNum" value="<?php echo(isset($_POST['req_phone']) ? forum_htmlencode($_POST['req_phone']) : '') ?>"
                           size="35" maxlength="16" required spellcheck="false"/>
                </span>
            </div>
    </div>
        <!--Подключаем плагин маску для поля тлф номера-->
        <script type="text/javascript" src="/theme/sp12/js/jquery-3.2.0.min.js"></script>
<!--        <script type="text/javascript" src="/theme/sp12/js/jquery.inputmask.min.js"></script> -->
        <script type="text/javascript" src="/theme/sp12/js/maskedInput.min.js"></script>
		
		<div>
            <input type="hidden" id="eField" name="eField" value=""/>
        </div>
        
<!-- todo   Пользователь по акции?-->
        <div class="sf-box text required">
            <label><span style="padding-left: 45px;">Регистрация по акции?</span></label>
            <select class="action" name="actionUser" style="margin-left: 15px;">
                <option value="0">Нет</option>
                <option value="1">Да</option>
            </select>
        </div>

<!-- todo Поле ввода промо-кода-->
<!--        <div class="sf-box text required">
            <div>
                <label><span style="padding-left: 126px;">Промо-код</span></label>
                <span style="margin-left: 15px;"><input name="promoCode" value="" /></span>
                <p><span style="margin-left: 215px;">Если вы регистрируетесь по акции, введите промо-код!</span>
            </div>
        </div>-->
...
<!-- 	Маска для ввода номера телефона! Отключена из-за подключения recaptcha ->
...
<?php ($hook = get_hook('rg_register_group_end')) ? eval($hook) : null; ?>
		<div class="frm-buttons">
			<span>
				<input id="checkSubmit" type="hidden" name="register" value="<?php echo $lang_profile['Register']?>"/>
			</span>
		</div>

<!--Блокируем кнопку "Регистрация", пока все поля не будут заполнены-->
<script type="text/javascript" src="/theme/sp12/js/submit_disable.js"></script>

<div style="margin-left: 70px;"><b>Еcли у вас не получается зарегистрироваться, напишите сообщение Whatsapp админу сайта... +7 964 417 3969</div>
