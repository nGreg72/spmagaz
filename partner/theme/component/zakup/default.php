...
<div  id="sort" style="border-bottom: 4px double gray"><b>Фильтр:</b>
	<a href="index.php?component=zakup" <?if(empty($_GET['status']) and empty($_GET['check'])):?><?endif?> class="filtr-all"> <big>Все</big> </a>
<div>
	<?foreach($statuslist as $it):?>
		<a href="index.php?component=zakup&status=<?=$it['id']?>" <?if($it['id']==$_GET['status']):?><?endif?> class="filtr"><?=$it['name'];?> </a>
	<?endforeach?>
</div>
</div>
...
<tr><td>Закупка</td>
	...
	<?if (count($items)==0):?><tr><td>С данным статусом закупок нет</td></tr><?endif?>
...
<tr style="background-color: rgb(239, 236, 236); border-bottom: 2px solid grey;">
	<td width="350">
			...
			<td><b><a href="/com/org/open/<?=$item['id']?>"><?=$item['title']?></a></b><br/><br/>
	...
	<td  valign="top" id="zp<?=$item['id']?>">
			<table border="0"><tr><td valign="top" width="130">
			  <ul style="margin-left:-10px;">
				<?if($item['status']==2):?>
				<li><a href="?component=zakup&section=add&id=<?=$item['id']?>" class="news_body_a">Принять</a></li>
				<li><a href="?component=zakup&section=del&id=<?=$item['id']?>" class="news_body_a">Удалить</a></li>
				<?endif?>
				<li><a href="/com/message/new/<?=$item['user']?>" class="news_body_a">Написать Оргию</a></li>
			  </ul>
			<td valign="top">
			<form action="?section=default&component=zakup&page=<?=$_GET['page']?>&status=<?=$_GET['status']?>&check=<?=$_GET['check']?>#zp<?=$item['id']?>" method="post">
	...
	<div>
		<div>Статус<br/>
			<select name="status" class="inputbox zakupStatus" rel="<?=$item['id']?>">
				<?foreach($statuslist as $it):?>
					<option value="<?=$it['id']?>" <?if($it['id']==$item['status']):?>selected<?endif?>><?=$it['name']; ?></option>
				<?endforeach?>
			</select>
		</div>
		...
		<div style="margin-top: 10px;">Организатор<br/>
			<select name="user" class="inputbox">
				<?foreach($orglist as $it):?>
					<option value="<?=$it['id']?>" <?if($it['id']==$item['user']):?>selected<?endif?>><?=$it['username']; ?></option>
				<?endforeach?>
			</select>
		</div>	
		<div style="margin-top: 10px; margin-left: 177px;"><input type="submit" value="Сохранить" class="mini"/></div>
	</div>
<!--			<a href="?component=zakup&section=del&id=<?=$item['id']?>&check=1" class="news_body_a">удалить из закрепленных</a><br/>
		...
		<td class="tab-td-1" width="500">
			<b>Организатор:</b> <a href="/com/profile/default/<?=$data[0]['user']?>"><?=$data[0]['username']?></a><br/><br/>
...
<!--			<a href="?component=zakup&section=del&id=<?=$item['id']?>&check=1" class="news_body_a">удалить из закрепленных</a><br/>-->
			<a href="?component=zakup&section=del&id=<?=$item['id']?>&check=2" class="news_body_a">удалить из закрепленных, с закупкой</a>
		</td></tr>
		<?}?>
	<?endforeach?>
</table>
...
<br/>
<div class="sort">
<a href="index.php?component=zakup&check=all" <?if(!empty($_GET['check'])):?>class="selected"<?endif?>>Смотреть список закрепленых закрепленых поставщиков за оргами</a>
</div>
