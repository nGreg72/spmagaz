...
//print_r($sql);exit;
	...
	$waUser=intval($_POST['waUser']);
	$alertmail=intval($_POST['alertmail']);
	$idd=intval($_POST['id']);
...
//Фильтр пользователей по: -логину -имени -почте -телефону	
    if  ($_POST['realname']>'') 
            $sql_filter= "and(punbb_users.username LIKE '%".PHP_slashes($_POST['realname'])."%' 
            OR punbb_users.realname LIKE '%".PHP_slashes($_POST['realname'])."%'
            OR `punbb_users`.`email` LIKE '%".PHP_slashes($_POST['realname'])."%' 
            OR `punbb_users`.`phone` LIKE '%".PHP_slashes($_POST['realname'])."%'
        )";
...
//---------------------------------------------
    ...
    if (isset($select_group)) {
        $groupFilter = "and group_id = $select_group";}

    $sql = "SELECT `punbb_users`.*, `group`.`name` 
			FROM `punbb_users` 
			LEFT JOIN `group` ON `punbb_users`.`group_id`=`group`.`fgid` 
			WHERE `punbb_users`.`id`>1
			$sql_filter  $groupFilter 
			order by id asc
			LIMIT $start, $num";
    $all = $DB->getAll($sql);
	...
	if($_POST['realname']>'')
        $sql_filter="WHERE (punbb_users.username LIKE '%".PHP_slashes($_POST['realname'])."%' 
        OR punbb_users.realname LIKE '%".PHP_slashes($_POST['realname'])."%')";
