...
class phpthumb {
	...
	function setSourceFilename($sourceFilename) {
		...
		if (is_null($this->config_output_format)) {
			...
			if (preg('^[a-z]{3,4}$', $sourceFileExtension)) {
...
//				if (preg('^bord\|([0-9]+)', $value, $matches)) {
...
//		}
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if (!$this->config_allow_src_above_phpthumb && !preg('^'.preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', dirname(__FILE__))), $AbsoluteFilename)) {
